...
from skimage.segmentation import clear_border  # (optional; not used everywhere)
...
def repair_and_smooth_mesh(mesh: trimesh.Trimesh, smooth_iters=6, strong=False, verbose=True):
    # light cleanup
    ...
    if strong:
        try:
            ...
            if verbose: print(f"[warn] strong repair skipped: {e}")
...
# ---------------------------------------------------------------------
# Helpers: ring mesh (visual marker) and clipping
# ---------------------------------------------------------------------
def make_ring_mesh(
    img_iso,
    center_zyx,
    ...
    center_zyx : (z, y, x) indices in the RESAMPLED volume
    radius_mm  : ring radius in millimeters
    axis       : which plane the ring lies in; 'z' => ring in XY plane
    """
    ...
    if axis == 'z':
        top_outer = circle_xy(r_out, cz_mm + h / 2)
        top_inner = circle_xy(r_in,  cz_mm + h / 2)
        bot_outer = circle_xy(r_out, cz_mm - h / 2)
        bot_inner = circle_xy(r_in,  cz_mm - h / 2)
    elif axis == 'y':
        top_outer = circle_xz(r_out, cy_mm + h / 2)
        top_inner = circle_xz(r_in,  cy_mm + h / 2)
        bot_outer = circle_xz(r_out, cy_mm - h / 2)
        bot_inner = circle_xz(r_in,  cy_mm - h / 2)
    else:  # 'x'
        top_outer = circle_yz(r_out, cx_mm + h / 2)
        top_inner = circle_yz(r_in,  cx_mm + h / 2)
        bot_outer = circle_yz(r_out, cx_mm - h / 2)
        bot_inner = circle_yz(r_in,  cx_mm - h / 2)
    ...
    for i in range(n):
        ...
        # inner wall (reverse so normals point outward)
...
def clip_mask_radius(mask_itk, img_iso, radius_mm, center_zyx=None, axis=None):
    """
    Clip mask to a sphere (axis=None) or to an infinite cylinder around axis ('x'/'y'/'z').
...
# ---------------------------------------------------------------------
# DICOM/NIfTI reading + isotropic resample
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Segmentation: lungs & airway (seeded)
# ---------------------------------------------------------------------
def segment_lungs(
    img_iso,
    hu_low=-1000,
    hu_high=-320,               # air-ish threshold (less strict)
    keep_components=2,
...
def segment_airway_seeded(img_iso, seed_zyx, low=-1024, high=-800, iters=2):
    ...
    # Morphological clean-up (arguments work across SimpleITK versions)
    rg = sitk.BinaryMorphologicalOpening(
        rg, kernelRadius=[1, 1, 1], foregroundValue=1, backgroundValue=0, safeBorder=True, kernelType=sitk.sitkBall
    )
...
# ---------------------------------------------------------------------
# Utilities: masking, bbox, meshing
# ---------------------------------------------------------------------
...
def mask_to_mesh_stl(mask_itk, out_path, decimate_ratio=0.5, smooth_iters=6, repair_strong=False, verbose=True):
    ...
    # basic tidy before decimation
    mesh.remove_degenerate_faces()
    mesh.remove_unreferenced_vertices()
    mesh.fix_normals()

    # decimate safely (keep fraction of faces)
    ...
    if not (n_faces < 2000 or r_keep >= 0.999):
        ...
        dec_fn = getattr(mesh, "simplify_quadratic_decimation", None) or getattr(mesh, "simplify_quadric_decimation", None)
        try:
            ...
            if verbose: print(f"[warn] decimation failed ({e}); continuing without decimation.")

    # repair + smooth + (optional) strong watertight fix
    mesh = repair_and_smooth_mesh(mesh, smooth_iters=int(smooth_iters), strong=bool(repair_strong), verbose=verbose)
...
def main():
    ...
    # Optional saves / utilities
    ap.add_argument("--save-lung-mask",  type=str, default=None, help="Write lung mask NIfTI (.nii.gz).")
    ...
    ap.add_argument("--outside-hu", type=float, default=-1024.0, help="HU value for outside-lung voxels in masked volume.")

    # Optional clip + visual ring
    ap.add_argument("--draw-circle", action="store_true", help="Overlay a thin ring into the output STL for visual guidance.")
    ap.add_argument("--radius-mm",  type=float, default=None,
                    help="Clip mask to this radius (mm). With --cyl-axis, uses a cylinder; otherwise a sphere.")
    ap.add_argument("--cyl-axis",   choices=['x', 'y', 'z'], default=None, help="Choose cylinder axis (x/y/z).")
    ...
    ap.add_argument("--smooth-iters", type=int, default=6, help="Taubin smoothing iterations (0 disables).")
    ap.add_argument("--repair-strong", action="store_true", help="Stronger watertight fix via pymeshfix (optional dep).")
    ...
    mask_to_mesh_stl(lungs_mask, args.out,
